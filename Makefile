...
linux/.config: linux/.git
	make -C linux ARCH=arm64 CROSS_COMPILE=aarch64-linux-gnu- LOCALVERSION=$(LOCALVERSION) clean CONFIG_ARCH_SUN50IW1P1=y
	make -C linux ARCH=arm64 CROSS_COMPILE=aarch64-linux-gnu- sun50iw1p1smp_linux_defconfig
	touch linux/.config

linux/arch/arm64/boot/Image: linux/.config
	make -C linux ARCH=arm64 CROSS_COMPILE=aarch64-linux-gnu- -j4 LOCALVERSION=$(LOCALVERSION) modules
	...
	make -C linux ARCH=arm64 CROSS_COMPILE=aarch64-linux-gnu- -j4 LOCALVERSION=$(LOCALVERSION) Image
...
busybox: busybox/.git
	...
	make -C busybox ARCH=arm64 CROSS_COMPILE=aarch64-linux-gnu- -j4 oldconfig
